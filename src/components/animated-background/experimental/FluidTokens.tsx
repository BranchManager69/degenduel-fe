...
export const FluidTokens: React.FC = () => {
  ...
  const tokenPointsRef = useRef<Array<{
    x: number;
    y: number;
    vx: number;
    vy: number;
  ...
  useEffect(() => {
    if (!canvasRef.current || !dimensions.width || !dimensions.height || !fluidRef.current) return;
    ...
    const diffuse = (_: number, x: Float32Array, x0: Float32Array, diff: number, dt: number) => {
    ...
    const diffuseRGB = (x: Float32Array, x0: Float32Array, diff: number, dt: number) => {
      const a = dt * diff * (GRID_SIZE - 2) * (GRID_SIZE - 2);
      ...
      for (let k = 0; k < ITERATIONS; k++) {
        for (let j = 1; j < GRID_SIZE - 1; j++) {
          for (let i = 1; i < GRID_SIZE - 1; i++) {
            ...
            for (let c = 0; c < 3; c++) {
              x[idx + c] = (x0[idx + c] + a * (
                x[idxRight + c] + x[idxLeft + c] +
                x[idxBottom + c] + x[idxTop + c]
              )) / (1 + 4 * a);
            }
          }
        }
    ...
    const advect = (_: number, d: Float32Array, d0: Float32Array, velocX: Float32Array, velocY: Float32Array, dt: number) => {
      let i0, i1, j0, j1;
      let x, y, s0, t0, s1, t1;
    ...
    const advectRGB = (d: Float32Array, d0: Float32Array, velocX: Float32Array, velocY: Float32Array, dt: number) => {
      let i0, i1, j0, j1;
      let x, y, s0, t0, s1, t1;
      ...
      for (let j = 1; j < GRID_SIZE - 1; j++) {
        for (let i = 1; i < GRID_SIZE - 1; i++) {
          ...
          const idx = index * 3;
          for (let c = 0; c < 3; c++) {
            d[idx + c] = s0 * (t0 * d0[IX(i0, j0) * 3 + c] + t1 * d0[IX(i0, j1) * 3 + c]) +
    ...
    const addSource = (dest: Float32Array, source: number, x: number, y: number, radius: number, strength: number) => {
      const gridPos = toGrid(x, y);
      const r = Math.max(1, Math.ceil(radius * GRID_SIZE / Math.max(dimensions.width, dimensions.height)));
    ...
    const addSourceRGB = (dest: Float32Array, r: number, g: number, b: number, x: number, y: number, radius: number, strength: number) => {
      const gridPos = toGrid(x, y);
      const r2 = Math.max(1, Math.ceil(radius * GRID_SIZE / Math.max(dimensions.width, dimensions.height)));
    ...
    const project = (velocX: Float32Array, velocY: Float32Array, p: Float32Array, div: Float32Array) => {
    ...
    const velocityStep = (velocX: Float32Array, velocY: Float32Array, velocX0: Float32Array, velocY0: Float32Array, dt: number) => {
      // Add forces
    ...
    const densityStep = (x: Float32Array, x0: Float32Array, velocX: Float32Array, velocY: Float32Array, dt: number) => {
      // Add sources
    ...
    const animate = () => {
      ...
      const {
        ...
        densityPrev, velocityXPrev, velocityYPrev
      } = fluidRef.current;
      ...
      tokenPointsRef.current.forEach(point => {
        ...
        addSource(velocityXPrev, point.vx * 5, point.x, point.y, 50, point.strength);
        addSource(velocityYPrev, point.vy * 5, point.y, point.y, 50, point.strength);
        
        // Add color to fluid
        addSourceRGB(
          densityPrev, 
          point.color[0], point.color[1], point.color[2], 
          point.x, point.y, 50, point.strength * 0.05
        );
      });
      ...
      for (let y = 0; y < dimensions.height; y++) {
        for (let x = 0; x < dimensions.width; x++) {
          ...
          // Update image data
          data[idxCanvas] = Math.max(0, Math.min(255, Math.floor(density[idx])));     // R
          data[idxCanvas + 1] = Math.max(0, Math.min(255, Math.floor(density[idx + 1]))); // G
          data[idxCanvas + 2] = Math.max(0, Math.min(255, Math.floor(density[idx + 2]))); // B
          
          // Calculate alpha based on overall density
          const totalDensity = density[idx] + density[idx + 1] + density[idx + 2];
