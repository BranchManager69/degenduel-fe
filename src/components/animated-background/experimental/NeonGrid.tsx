...
export const NeonGrid: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const nodes = sortedTokens.map((token) => {
      ...
      let color;
      if (change5m > 1) { // Strong positive in short term
        color = "#00ff88"; // Bright green
      } else if (change5m > 0) { // Moderate positive in short term
        color = "#00ccff"; // Cyan
      } else if (change5m > -1) { // Moderate negative in short term
        color = "#ff44aa"; // Pink
      } else { // Strong negative in short term
        color = "#ff0044"; // Red
      }
      
      // Random position on grid
      const randomGridX = Math.floor(Math.random() * gridRef.current.verticalLines.length);
      const randomGridY = Math.floor(Math.random() * gridRef.current.horizontalLines.length);
      ...
      if (directed) {
        const targetGridX = Math.floor(Math.random() * gridRef.current.verticalLines.length);
        const targetGridY = Math.floor(Math.random() * gridRef.current.horizontalLines.length);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      horizontalLines.forEach(line => {
        drawNeonLine(
          0, line.y,
          dimensions.width, line.y,
          line.color, line.width, line.glow
        );
      });
      ...
      nodes.forEach(node => {
        ...
        if (node.directed && node.targetX !== undefined && node.targetY !== undefined && node.speed !== undefined) {
          ...
          if (distance > 5) {
            ...
            const targetGridX = Math.floor(Math.random() * verticalLines.length);
            const targetGridY = Math.floor(Math.random() * horizontalLines.length);
        ...
        drawNeonNode(
          node.x, node.y,
          node.size, node.color, node.glow,
          node.phase, node.symbol
        );
      });
