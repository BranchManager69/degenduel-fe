...
export const AbstractPatterns: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const positiveTokens = tokens.filter(token => 
      parseFloat(token.change24h || '0') > 0
    ).slice(0, 8); // Limit to prevent performance issues
    
    const negativeTokens = tokens.filter(token => 
      parseFloat(token.change24h || '0') < 0
    ).slice(0, 8);
    ...
    for (let i = 0; i < negativeTokens.length; i++) {
      ...
      segments.push({
        ...
        endPoint: [
          dimensions.width / 2 + Math.cos(angle + Math.PI) * dimensions.width * 0.4,
          dimensions.height / 2 + Math.sin(angle + Math.PI) * dimensions.height * 0.4
        ],
    ...
    for (let i = 0; i < highCapTokens.length; i++) {
      ...
      const radius = Math.max(20, Math.min(100, dimensions.width * 0.05 * radiusFactor));
      ...
      if (change > 0) {
        ...
        color = [200 + (intensity * 55), 30 + (intensity * 25), 60 + (intensity * 20), 0.6];
      }
    ...
    for (let i = 0; i < remainingTokens.length; i++) {
      ...
      const centerX = dimensions.width / 2 + Math.cos(angle) * dimensions.width * distanceFactor;
      const centerY = dimensions.height / 2 + Math.sin(angle) * dimensions.height * distanceFactor;
      
      // Size based on market cap
      const sizeFactor = Math.log10(marketCap) / 12;
      const size = Math.max(30, Math.min(120, dimensions.width * 0.05 * sizeFactor));
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      segments.forEach(segment => {
        ...
        // Draw wavy line
        drawWavyLine(
          segment.startPoint[0], segment.startPoint[1],
