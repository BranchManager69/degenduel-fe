    ...
    <Card className="bg-dark-200/50 backdrop-blur-sm border-dark-300">
      <CardHeader>
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-100">Your Portfolio</h3>
      ...
      <CardContent>
        <div className="space-y-4">
          {tokens.map(({ token, amount, initialValue, currentValue }) => {
            ...
            const formattedAmount = token.price >= 0.01 
              ? amount.toFixed(2)
              : amount.toFixed(0);
            ...
            return (
              <div key={token.symbol} className="p-4 rounded-lg bg-dark-300/50">
                <div className="flex justify-between items-center mb-3">
                  <div>
                    <div className="text-lg font-medium text-gray-100">{token.name}</div>
                    <div className="text-sm text-gray-400">{formattedAmount} {token.symbol}</div>
                  </div>
                  <div className="text-right">
                ...
                <div className="relative h-2 bg-dark-400 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min(Math.max((change + 100) / 2, 0), 100)}%` }}
                  />
                </div>
              </div>
            );
