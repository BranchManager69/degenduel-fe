// LEGACY IMPLEMENTATION - Saved for future reference
// This component has been replaced with a simpler Logo component
...
const Logo3DLegacy: React.FC<Logo3DProps> = ({
  size = "lg",
  className = "",
}) => {
  ...
  useEffect(() => {
    ...
    const redCandle = createCandlestick(
      0xff2266,
      new THREE.Vector3(-0.5, -3, 0),
      0.5,
      1,
    );
    const greenCandle = createCandlestick(
      0x22ff66,
      new THREE.Vector3(0.5, -3, 0),
      0.5,
      1,
    ...
    manager.registerAnimation(componentId, (deltaTime) => {
      ...
      d2Group.rotation.y =
        Math.sin(animationTime * 0.7 + Math.PI) * rotationAmount;
