...
export const IpBanCheckForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to check IP status",
      );
    } finally {
      setLoading(false);
    }
              ...
              {banResult.banDetails && (
                <div className="mt-2 text-xs text-gray-400">
                  <p>
                    ...
                    {banResult.banDetails.reason.substring(0, 50)}...
                  </p>
                  <p>
                    <span className="text-gray-500">Type:</span>{" "}
                    {banResult.banDetails.is_permanent
                      ? "Permanent"
                      : "Temporary"}
                  </p>
                  {!banResult.banDetails.is_permanent &&
                    banResult.banDetails.expires_at && (
                      <p>
