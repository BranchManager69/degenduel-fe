...
export const ServiceGrid: React.FC = () => {
  ...
  // Group nodes by type
  const groupedNodes = skyDuel.nodes.reduce((acc, node) => {
    if (!acc[node.type]) {
      acc[node.type] = [];
    }
      ...
      {Object.entries(groupedNodes).map(([type, nodes]) => (
        <div key={type} className="mb-6">
          <h3 className="text-lg font-semibold text-brand-100 mb-3 flex items-center">
            <span className="mr-2">{getNodeTypeIcon(type as ServiceNode["type"])}</span>
            ...
            {nodes.map((node) => (
              <div
                ...
                className={`rounded-lg border ${getStatusColor(node.status)} p-4 transition-all hover:scale-[1.02] cursor-pointer ${
                  node.id === skyDuel.selectedNode ? "ring-2 ring-brand-400" : ""
                }`}
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-2 ${getStatusDot(node.status)}`}></div>
                      <h4 className="font-medium text-brand-100">{node.name}</h4>
                    </div>
                    <div className="text-xs text-gray-400 mt-1">ID: {node.id}</div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="text-right text-brand-300">{formatUptime(node.uptime)}</div>
                  
                  <div className="text-gray-400">Connections:</div>
                  <div className="text-right text-brand-300">{node.metrics.connections}</div>
                  
                  <div className="text-gray-400">Error Rate:</div>
                  <div className="text-right text-brand-300">{node.metrics.errorRate}%</div>
                </div>
