...
export const ServiceDetails: React.FC<ServiceDetailsProps> = ({ nodeId }) => {
  ...
  const getConnectionStatusColor = (status: "active" | "degraded" | "failed") => {
    switch (status) {
      case "active":
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="p-3 rounded-lg bg-dark-700/50">
          ...
          <div className="text-brand-300 font-medium">{formatUptime(node.uptime)}</div>
        </div>
      </div>
      
      <div className="mb-4">
        <h4 className="text-sm uppercase tracking-wider text-gray-400 mb-2">Metrics</h4>
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-lg bg-dark-700/50">
            <div className="text-sm text-gray-400 mb-1">CPU Usage</div>
            <div className="text-brand-300 font-medium">{node.metrics.cpu}%</div>
          </div>
          
          <div className="p-3 rounded-lg bg-dark-700/50">
            <div className="text-sm text-gray-400 mb-1">Memory Usage</div>
            <div className="text-brand-300 font-medium">{node.metrics.memory}%</div>
          </div>
          
          <div className="p-3 rounded-lg bg-dark-700/50">
            <div className="text-sm text-gray-400 mb-1">Active Connections</div>
            <div className="text-brand-300 font-medium">{node.metrics.connections}</div>
          </div>
          
          <div className="p-3 rounded-lg bg-dark-700/50">
            <div className="text-sm text-gray-400 mb-1">Requests Per Minute</div>
            <div className="text-brand-300 font-medium">{node.metrics.requestsPerMinute}</div>
          </div>
          ...
          <div className="p-3 rounded-lg bg-dark-700/50 col-span-2">
            ...
            <div className="flex items-center">
              ...
              <span className="text-brand-300 font-medium">{node.metrics.errorRate}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Timestamps */}
      <div className="mb-4">
        <h4 className="text-sm uppercase tracking-wider text-gray-400 mb-2">Timeline</h4>
        <div className="text-sm">
          <div className="flex justify-between mb-1">
            <span className="text-gray-400">Last Restart:</span>
            <span className="text-gray-300">{formatDate(node.lastRestart)}</span>
          </div>
        </div>
      </div>
          ...
          <div className="mb-3">
            ...
            <div className="space-y-2">
              {connectedTo.map(({ connection, node }) => (
                ...
                >
                  <div className="flex items-center">
                    <span className={`mr-2 ${getConnectionStatusColor(connection.status)}`}>→</span>
                    <span className="text-gray-300">{node?.name || connection.target}</span>
                  </div>
            ...
            <div className="space-y-2">
              {connectedFrom.map(({ connection, node }) => (
                ...
                >
                  <div className="flex items-center">
                    <span className={`mr-2 ${getConnectionStatusColor(connection.status)}`}>←</span>
                    <span className="text-gray-300">{node?.name || connection.source}</span>
                  </div>
                  <div className="text-xs text-gray-400">
                    {connection.latency}ms | {connection.throughput}/s
                  </div>
