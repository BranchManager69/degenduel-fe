...
export const ServiceCircuitView: React.FC = () => {
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-gray-100 mb-3">Circuit Breaker Status</h2>
        ...
        <div className={`p-4 rounded-lg border ${
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm">
              {skyDuel.nodes.filter(n => n.status !== "online").length} service{
                skyDuel.nodes.filter(n => n.status !== "online").length !== 1 ? "s" : ""
              } in degraded state
            </span>
          </div>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold text-gray-200">{node.name}</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      ...
                      <span className="text-sm text-gray-300">{circuitState.toUpperCase()}</span>
                    </div>
                  </div>
                  
                  {/* Circuit Health */}
                  <div>
                    <span className="text-sm text-gray-400">Circuit Health</span>
                    <div className="mt-1">
                      <div className="flex justify-between mb-1">
                        <span className="text-sm text-gray-300">Error Rate: {node.metrics.errorRate}%</span>
                        <span className="text-sm text-gray-300">{healthProgress}% Healthy</span>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="text-right text-brand-300">{node.metrics.requestsPerMinute}</div>
                    
                    <div className="text-gray-400">CPU:</div>
                    <div className="text-right text-brand-300">{node.metrics.cpu}%</div>
                    
                    <div className="text-gray-400">Memory:</div>
                    <div className="text-right text-brand-300">{node.metrics.memory}%</div>
                    
                    <div className="text-gray-400">Connections:</div>
                    <div className="text-right text-brand-300">{node.metrics.connections}</div>
                  </div>
