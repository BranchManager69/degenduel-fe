...
export const LiveUserActivityMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
        ...
        try {
          ...
          if (data.type === "user_activity_update") {
            ...
            Object.entries(SITE_ZONES).forEach(([zone, _]) => {
              const newCount = newUsers.filter(
                (u: UserActivity) => u.current_zone === zone,
              ).length;
              const prevCount = prevUsers.filter(
                (u) => u.current_zone === zone,
              ).length;
      ...
      ws.onclose = (event) => {
        console.log(
          "Disconnected from analytics websocket",
          event.code,
          event.reason,
        );
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="space-y-2">
            {(
              Object.entries(thresholds.SOUND_VOLUME) as [
                SoundVolumeKey,
                number,
              ][]
            ).map(([key, value]) => (
      ...
      <div className="mt-4 grid grid-cols-3 gap-4">
        ...
        <div className="bg-dark-300/30 rounded-lg p-3">
          ...
          <p className="text-xl font-semibold text-gray-100">
            {Object.entries(SITE_ZONES).reduce((max, [key, _]) => {
              ...
              return count > users.filter((u) => u.current_zone === max).length
                ? key
                : max;
        ...
        <div className="bg-dark-300/30 rounded-lg p-3">
          ...
          <p className="text-xl font-semibold text-gray-100">
            ◎{" "}
            {users
              .reduce((sum, user) => sum + user.wallet_balance, 0)
              .toFixed(2)}
          </p>
        </div>
      </div>
            ...
            <table className="w-full">
              <tbody>
                {adminLogs.slice(0, 5).map((log) => (
                  <tr
                    key={log.id}
                    className="border-b border-dark-300/50 last:border-0"
                  >
