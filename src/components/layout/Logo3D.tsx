...
export const Logo3D: React.FC<Logo3DProps> = ({ isCompact = false }) => {
  ...
  useEffect(() => {
    ...
    let threeManager = ThreeManager.getInstance();
  ...
  const setupLogoScene = (scene: THREE.Scene | undefined, isCompact: boolean) => {
    if (!scene) return;
  ...
  const animateLogo = (scene: THREE.Scene | undefined, deltaTime: number) => {
    ...
    const animateGroup = (object: THREE.Object3D, phaseOffset = 0) => {
      ...
      if (initialPos && initialPos instanceof THREE.Vector3) {
        ...
        object.position.y = initialPos.y + Math.sin(now * 0.001 + phaseOffset) * 0.05;
      }
      
      // Animate children (letters)
      object.children.forEach((child, index) => {
        if (child instanceof THREE.Mesh) {
          child.rotation.z = Math.sin(now * 0.001 + index * 0.5 + phaseOffset) * 0.05;
        }
      });
    };
