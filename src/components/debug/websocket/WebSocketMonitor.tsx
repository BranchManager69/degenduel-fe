...
export const WebSocketMonitor: React.FC = () => {
  ...
  useEffect(() => {
    const types = Array.from(new Set(events.map((e) => e.socketType))).filter(
      Boolean,
    );
  ...
  const filteredEvents = events
    ...
    .filter(
      (event) =>
        socketTypeFilter === "all" || event.socketType === socketTypeFilter,
    );
      ...
      <div className="events-container h-96 overflow-y-auto bg-dark-300 rounded-md p-2 border border-gray-700">
        {filteredEvents.length === 0 ? (
          <div className="text-center py-4 text-gray-500">
            No WebSocket events recorded. Connect to WebSockets to see data
            here.
          </div>
        ) : (
            ...
            >
              <div className="event-header flex justify-between items-center">
                ...
                <div className="flex items-center">
                  <span
                    className={`socket-type text-xs font-mono mr-2 ${getSocketTypeColor(event.socketType)}`}
                  >
                    {event.socketType}
                  </span>
                  <span
                    className={`event-type text-xs font-bold ${getEventTypeColor(event.type)}`}
                  >
                    {event.type}
                  </span>
                </div>
