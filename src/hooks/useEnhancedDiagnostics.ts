...
export const useEnhancedDiagnostics = () => {
  ...
  const reconnect = () =>
    console.log("Reconnecting to diagnostics websocket...");
  ...
  const markErrorResolved = async (errorId: string) => {
    try {
      const response = await fetch(
        `/api/superadmin/diagnostics/errors/${errorId}/resolve`,
        {
          method: "POST",
        },
      );
  ...
  const disconnectSession = async (sessionId: string) => {
    try {
      const response = await fetch(
        `/api/superadmin/diagnostics/sessions/${sessionId}/disconnect`,
        {
          method: "POST",
        },
      );
