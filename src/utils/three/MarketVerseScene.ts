...
export class MarketVerseScene {
  ...
  private handleMouseMove(event: MouseEvent): void {
    ...
    const rect = this.camera.aspect === window.innerWidth / window.innerHeight 
      ? { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight }
      : (document.querySelector('#market-verse-container')?.getBoundingClientRect() || 
        { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight });
  ...
  private update(_deltaTime: number): void {
    ...
    if (this.highlightedSphere && (!intersects.length || this.highlightedSphere !== intersects[0].object)) {
      const material = this.highlightedSphere.material as THREE.MeshPhongMaterial;
    ...
    this.spheres.forEach(sphere => {
      ...
      sphere.position.y += Math.sin(Date.now() * 0.001 + sphere.position.x) * 0.05;
    });
  }
