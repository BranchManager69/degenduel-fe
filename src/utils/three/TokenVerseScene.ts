...
export class TokenVerseScene {
  ...
  private handleMouseMove(event: MouseEvent): void {
    ...
    const rect = this.camera.aspect === window.innerWidth / window.innerHeight 
      ...
      : (document.querySelector('#token-verse-container')?.getBoundingClientRect() || 
        { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight });
  ...
  private setupScene(): void {
    ...
    const lights = [
      ...
      { color: 0xff8800, position: [-50, -50, -50] as [number, number, number] },
      { color: 0x00ff88, position: [-50, 50, -50] as [number, number, number] },
    ];
    
    lights.forEach(({ color, position }) => {
      const light = new THREE.PointLight(color, this.settings.intensity / 25, 300);
  ...
  public updateSettings(settings: Partial<typeof this.settings>): void {
    ...
    if (settings.starIntensity !== undefined || settings.particleCount !== undefined) {
      this.createBackgroundParticles();
    }
