...
const UserDiagnostics: React.FC = () => {
  ...
  const actualDiagnosticsData = diagnosticsData.activeSessions.length
    ? diagnosticsData
    : sampleData;
      ...
      <div className="container mx-auto p-8 relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            Advanced monitoring and diagnostic tools for user activity and
            system interactions
          </p>
        ...
        <div
          className={`mb-8 ${error ? "bg-red-500/10 border-red-500/20" : connected ? "bg-purple-500/10 border-purple-500/20" : "bg-yellow-500/10 border-yellow-500/20"} border rounded-lg p-4 flex items-center justify-between`}
        >
          <div>
            <h2
              className="text-lg font-semibold mb-1"
              style={{
                color: error ? "#f87171" : connected ? "#c084fc" : "#fbbf24",
              }}
            >
              {error
                ? "Connection Error"
            ...
            >
              {error ||
                (connected
                  ? "Real-time diagnostics data is streaming."
                  : "Establishing connection to diagnostics service...")}
            </p>
          </div>
          <div
            className={`rounded-full p-3 ${error ? "bg-red-500/20" : connected ? "bg-purple-500/20" : "bg-yellow-500/20"}`}
          >
            {connected ? (
              <svg
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-3 bg-purple-500/20 rounded-lg">
              ...
              <div className="text-2xl font-bold text-purple-100">
                {
                  actualDiagnosticsData.activeSessions.filter((s) => s.active)
                    .length
                }
              </div>
            </div>
            <div className="p-3 bg-purple-500/20 rounded-lg">
              ...
              <div className="text-2xl font-bold text-purple-100">
                {
                  actualDiagnosticsData.userErrors.filter((e) => !e.resolved)
                    .length
                }
              </div>
            </div>
            <div className="p-3 bg-purple-500/20 rounded-lg">
              ...
              <div className="text-2xl font-bold text-purple-100">
                {
                  Object.values(actualDiagnosticsData.walletDiagnostics).filter(
                    (w) => w.status !== "healthy",
                  ).length
                }
              </div>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-dark-300/50 text-left text-xs text-gray-400">
                        ...
                        <th className="p-3 border-b border-dark-300">
                          Started
                        </th>
                        <th className="p-3 border-b border-dark-300">
                          Duration
                    ...
                    <tbody className="divide-y divide-dark-300/70">
                      {actualDiagnosticsData.activeSessions.map((session) => (
                        <tr
                          ...
                          className="hover:bg-dark-300/30 text-sm"
                        >
                          ...
                          <td className="p-3 whitespace-nowrap">
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${session.active ? "bg-green-500/20 text-green-300" : "bg-gray-500/20 text-gray-400"}`}
                            >
                ...
                <div className="p-8 bg-dark-300/50 rounded-lg flex items-center justify-center">
                  <div className="text-gray-400 flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                      User sessions will appear here when users connect
                    </p>
                  </div>
                </div>
              )}
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="flex items-center space-x-2 mb-2">
                            <div
                              className={`h-2 w-2 rounded-full ${error.resolved ? "bg-gray-500" : "bg-red-500"}`}
                            ></div>
                          ...
                          <div className="text-xs text-gray-400">
                            ...
                            {error.wallet.substring(0, 6)}...
                            {error.wallet.substring(error.wallet.length - 4)}
                          </div>
                        </div>
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-100 mb-4">
                ...
                <span className="ml-2 text-xs bg-yellow-500/20 text-yellow-400 px-2 py-1 rounded-full">
                  {
                    Object.values(
                      actualDiagnosticsData.walletDiagnostics,
              ...
              ) : Object.keys(actualDiagnosticsData.walletDiagnostics).length >
                0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {Object.values(actualDiagnosticsData.walletDiagnostics).map(
                    (wallet) => (
                      <div
                        key={wallet.wallet}
                ...
                <div className="p-8 bg-dark-300/50 rounded-lg flex items-center justify-center">
                  <div className="text-gray-400 flex flex-col items-center">
                    ...
                    <p className="text-center">
                      No wallet diagnostics available
                    </p>
                    <p className="text-sm text-gray-500 mt-2">
                      Wallet data will appear when users connect
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Basic user monitoring capabilities
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Real-time session tracking with activity timelines
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  User-specific error and exception tracking
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                <h3 className="text-purple-300 font-medium">
                  Wallet Diagnostics (Planned)
                </h3>
                <p className="text-gray-400 text-sm">
                  Advanced wallet debugging and transaction monitoring tools
                </p>
              </div>
            </div>
          </div>
