...
import { FaNetworkWired, FaToolbox, FaDiagnoses, FaTachometerAlt, FaCode, FaHistory } from "react-icons/fa";
...
const WebSocketHub: React.FC = () => {
  ...
  const tools: ToolCard[] = [
    {
      ...
      description: "Monitor WebSocket connections, run tests, and view live message traffic",
    ...
    },
    {
      ...
      description: "Comprehensive dashboard for monitoring and controlling all WebSocket services",
  ...
  const accessibleTools = tools.filter(tool => 
    tool.access === "admin" || (tool.access === "superadmin" && user?.is_superadmin)
  );
  ...
  return (
    <div className="container mx-auto p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-100">WebSocket Tools Hub</h1>
        <p className="text-gray-400 mt-2">
          Central access point for all WebSocket monitoring, debugging, and management tools
        </p>
      </div>
      ...
      <div className="bg-dark-200/50 backdrop-blur-sm rounded-lg p-6 border border-brand-500/20 mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-100">WebSocket System Overview</h2>
          <FaNetworkWired className="w-8 h-8 text-brand-400" />
        </div>
        <p className="text-gray-400 mb-4">
          DegenDuel uses a hub-and-spoke WebSocket architecture with centralized connection management 
          and specialized hooks for specific service needs. This hub provides access to all WebSocket
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-dark-300/30 rounded-lg p-4">
            <h3 className="font-semibold text-brand-400 mb-2">Admin Services</h3>
            <ul className="text-sm text-gray-300 space-y-1">
        ...
        {accessibleTools.map((tool, index) => (
          ...
          >
            ...
            <div className="flex items-center mb-4">
              <div className="mr-4 text-brand-400">
                {tool.icon}
              </div>
              <h3 className="text-xl font-semibold text-gray-100">{tool.title}</h3>
            </div>
            <p className="text-gray-400 text-sm mb-4">{tool.description}</p>
            {tool.isDeprecated && tool.replacedBy && (
              <p className="text-yellow-300 text-xs">
                Please use <Link to={tool.replacedBy} className="underline">{
                  tools.find(t => t.path === tool.replacedBy)?.title || 'recommended alternative'
                }</Link> instead.
              </p>
            )}
            {tool.access === "superadmin" && (
              <div className="text-xs text-gray-500 mt-2">Requires SuperAdmin access</div>
            )}
      ...
      <div className="mt-8 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
        <h3 className="text-lg font-semibold text-blue-400 mb-2">Standardization Initiative</h3>
        <p className="text-gray-300 text-sm">
          We're currently standardizing our WebSocket tools and interfaces. This hub will be the central
          access point for all WebSocket-related functionality. If you encounter any issues or have
          suggestions for improvement, please contact the DevOps team.
        </p>
      </div>
    </div>
  );
