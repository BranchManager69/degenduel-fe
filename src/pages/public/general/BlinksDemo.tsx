...
export const BlinksDemo: React.FC = () => {
  ...
  const handleError = (error: Error) => {
    // Call toast.error directly - React hooks can be called in event handlers
    try {
      toast.error(error.message || 'Transaction failed');
    } catch (err) {
      console.error('Failed to show toast:', err);
      // Fallback to console error if toast fails
      console.error('Transaction error:', error.message || 'Transaction failed');
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white mb-6">Purchase Contest Credits</h2>
          
          <div className="space-y-6">
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              <h3 className="text-xl font-bold text-white mb-2">Contest Credits</h3>
              <p className="text-gray-300 mb-4">Burn DUEL tokens to get contest credits for creating contests</p>
              
              <div className="flex items-center justify-between text-sm text-gray-400 mb-6">
                <div>Cost: 69,420 DUEL</div>
                <div>Receives: 1 Credit</div>
              </div>
              
              <BlinkButton
                blinkUrl="/api/blinks/purchase-contest-credit"
                params={{
                  tokenAmount: "69420",
                  creditAmount: "1"
                }}
                ...
                onSuccess={(signature) => toast.success(`Credit purchased! Signature: ${signature.substring(0, 8)}...`)}
                onError={handleError}
              />
            </div>
          </div>
