...
export const DegenLevelPage: React.FC = () => {
  ...
  const currentUserNextLevelXp = achievements?.userProgress?.nextLevelThreshold || 100;
  ...
  const rankingsWithUser = React.useMemo(() => {
    ...
    const userEntry: UserLevel = {
      ...
      username: user.nickname || user.wallet_address.slice(0, 6) + '...' + user.wallet_address.slice(-4),
    ...
    const combinedRankings = [...topUsers, userEntry].sort((a, b) => 
      filter === 'level' ? b.level - a.level : b.achievements_count - a.achievements_count
    );
  ...
  }, [topUsers, user, isAuthenticated, currentUserLevel, currentUserXp, currentUserNextLevelXp, achievements, filter]);
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="space-y-8">
            ...
            <div className="text-center">
              <h1 className="text-4xl font-bold text-white mb-2">Degen Level Rankings</h1>
              <p className="text-gray-400 max-w-2xl mx-auto">
                Level up by participating in contests, earning achievements, and being active on DegenDuel. 
                Higher levels unlock better rewards and discounts!
              </p>
            </div>
                ...
                <div className="relative z-10">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="text-center md:text-left">
                      <h2 className="text-2xl font-bold text-white mb-1">Your Degen Profile</h2>
                      <div className="flex items-center justify-center md:justify-start mb-2">
                        <div className="bg-brand-500/20 px-3 py-1 rounded-full flex items-center">
                          <span className="text-brand-400 font-bold mr-1">Level {currentUserLevel}</span>
                          <span className="text-gray-400 text-sm">• {achievements?.unlockedAchievements?.length || 0} Achievements</span>
                        </div>
                      </div>
                      
                      <div className="text-gray-400 text-sm mb-4">
                        <p>XP: {currentUserXp} / {currentUserNextLevelXp}</p>
                      ...
                      <div className="bg-dark-300/30 rounded-lg p-3 mb-2 max-w-md">
                        <h3 className="text-brand-400 font-semibold mb-1">Token Listing Discount</h3>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400">Your discount:</span>
                          <span className="text-white font-bold">{discountPercent}%</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400">Listing fee with discount:</span>
                          <span className="text-white font-bold">{finalCost.toFixed(4)} SOL</span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="relative">
                        <div className="w-24 h-24 rounded-full bg-dark-300/50 flex items-center justify-center overflow-hidden border-4 border-brand-500/30">
                          {user?.profile_image?.url ? (
                            <img 
                              src={user.profile_image.thumbnail_url || user.profile_image.url} 
                        ...
                        <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-full bg-dark-200 border border-brand-500/30 flex items-center justify-center font-bold text-brand-400">
                          #{rankingsWithUser.find(u => u.wallet_address === user.wallet_address)?.rank || '?'}
                        </div>
                      </div>
            ...
            <Card className="bg-dark-200/50 backdrop-blur-lg border border-brand-500/20 p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button 
                    variant={filter === 'achievements' ? 'primary' : 'secondary'} 
                    size="sm"
              ...
              {loading ? (
                <div className="space-y-4">
                  {[...Array(5)].map((_, i) => (
                    <div key={i} className="animate-pulse flex items-center p-3 rounded-lg bg-dark-300/30">
                ...
                <div className="space-y-2">
                  {rankingsWithUser.map((userRank) => {
                    const isCurrentUser = isAuthenticated && userRank.wallet_address === user?.wallet_address;
                    return (
                      <div 
                      ...
                      >
                        <div className="w-10 text-center font-bold text-gray-400">#{userRank.rank}</div>
                        <div className="w-10 h-10 rounded-full bg-dark-300/50 flex items-center justify-center overflow-hidden border-2 border-dark-400/30 mr-3">
                          {userRank.profile_image?.url ? (
                            <img 
                              src={userRank.profile_image.thumbnail_url || userRank.profile_image.url} 
                        ...
                        <div className="flex-1">
                          <div className="font-semibold text-white">{userRank.username}</div>
                        ...
                        <div className="bg-dark-300/50 px-3 py-1 rounded-full text-sm">
                          <span className={`font-semibold ${filter === 'level' ? 'text-brand-400' : 'text-brand-300'}`}>
                            {filter === 'level' 
            ...
            {!isAuthenticated && (
              <div className="text-center mt-8">
                <p className="text-gray-400 mb-4">
                  Connect your wallet to see your Degen Level and unlock discounts!
                </p>
                <Link to="/login">
